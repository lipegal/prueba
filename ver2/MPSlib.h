...
Complex Norm_(ComplexMatrix& A,int site_information){
		...
		Single_Site_Operator_Procedure(O,A,site_information);
...
//Calculates the number of particles depending on 
Real Number_Particles(ComplexMatrix& A,int site, int sigma){
	//Sigma {-1,0,1} -> {0,1,2} --(sigma=3 for total particle number)
		Real number;
		int Ar=A.size1(), Ac=A.size2();
		...
		if(sigma==3){
			...
			O=small_number_Op(sigma);
		}
		
		Single_Site_Operator_Procedure(O,Aket,site);
